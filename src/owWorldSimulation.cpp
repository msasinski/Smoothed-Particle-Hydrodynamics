...
void display(void)
{
    ...
    for(i = 0; i<PARTICLE_COUNT; i++)
    {
        ...
        if((unsigned int)p_cpp[i*4 + 3] != BOUNDARY_PARTICLE /*&& (int)p_cpp[i*4 + 3] != ELASTIC_PARTICLE*/)
        {
            glBegin(GL_POINTS);
        ...
        if((j=(int)ec_cpp[ 4 * i_ec + 0 ])>=0)
        {
            ...
            if(i<j)
            {
                ...
                if(ec_cpp[ 4 * i_ec + 2 ]>1.f)//muscles
                {
                    ...
                    else if(ec_cpp[4*i_ec+2]-floor(ec_cpp[4*i_ec+2])>0.35f)
                    {
                        if(muscle_activation_signal_cpp[ (int)(floor( ec_cpp[4*i_ec+2])-1) ]>0.1)
                            glLineWidth((GLfloat)6.0);
                        else glLineWidth((GLfloat)2.0);
                    ...
                    else if(ec_cpp[4*i_ec+2]-floor(ec_cpp[4*i_ec+2])>0.25f)
                    {
                        if(muscle_activation_signal_cpp[ (int)(floor( ec_cpp[4*i_ec+2])-1) ]>0.1)
                            glLineWidth((GLfloat)6.0);
                        else glLineWidth((GLfloat)2.0);
                    ...
                    else if(ec_cpp[4*i_ec+2]-floor(ec_cpp[4*i_ec+2])>0.15f)
                    {
                        if(muscle_activation_signal_cpp[ (int)(floor( ec_cpp[4*i_ec+2])-1) ]>0.1)
                            glLineWidth((GLfloat)6.0);
                        else glLineWidth((GLfloat)2.0);
