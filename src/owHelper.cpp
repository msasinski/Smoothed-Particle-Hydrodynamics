        ...
        while(elasticLayers<=2)//this number defines number of radial muscle layers. can be 1, 2, 3 or more
        {
            ...
            if(wormBodyRadius>0)
                if(elasticLayers>=2)
                {
                    ...
                    else if(wormBodyRadius<r0*(1.00-0.707))
                    {
                        if(stage==1)
                        {
                            positionVector = position_cpp + 4 * (pCount+i_start);
                    ...
                    else
                    {
                        ...
                        if(j>jmin)
                        {
                            ...
                            for(q=0; q<prevSlice_pCount; q++)
                            {
                                if(q==0)
                                {
                                    ii=prevSlice_start+0;
                                    jj=prevSlice_start+2;
                                }
                                else if(q==1)
                                {
                                    ii=prevSlice_start+0;
                                    jj=prevSlice_start+3;
                                }
                                else if(q==2)
                                {
                                    ii=prevSlice_start+1;
                                    jj=prevSlice_start+4;
                                }
                                else if(q==3)
                                {
                                    ii=prevSlice_start+1;
                                    jj=prevSlice_start+5;
                                }
                                else
                                {
                                    ii=prevSlice_start+q-2;
                                    jj=prevSlice_start+q+2*(q+2<prevSlice_pCount);
                                }
                            ...
                            for(q=0; q<currSlice_pCount; q++)
                            {
                                if(q==0)
                                {
                                    ii=currSlice_start+0;
                                    jj=currSlice_start+2;
                                }
                                else if(q==1)
                                {
                                    ii=currSlice_start+0;
                                    jj=currSlice_start+3;
                                }
                                else if(q==2)
                                {
                                    ii=currSlice_start+1;
                                    jj=currSlice_start+4;
                                }
                                else if(q==3)
                                {
                                    ii=currSlice_start+1;
                                    jj=currSlice_start+5;
                                }
                                else
                                {
                                    ii=currSlice_start+q-2;
                                    jj=currSlice_start+q+2*(q+2<currSlice_pCount);
                                }
...
void owHelper::generateConfiguration(int stage, float *position_cpp, float *velocity_cpp, float *& elasticConnectionsData_cpp, int *membraneData_cpp, int & numOfLiquidP, int & numOfElasticP, int & numOfBoundaryP, int & numOfElasticConnections, int & numOfMembranes, int *particleMembranesList_cpp)
{
    ...
    if(stage==0)
    {
        ...
        if(numOfElasticP<=0) elasticConnectionsData_cpp = NULL;
        else elasticConnectionsData_cpp = new float[ 4 * numOfElasticP * MAX_NEIGHBOR_COUNT ];
    }
        ...
        for(i=numOfElasticP-numOfMembraneParticles; i<numOfElasticP; i++)
        {
            ...
            for(j=0; j<numOfElasticP+numOfLiquidP+numOfBoundaryP; j++)
            {
                if(i!=j)
                {
                    ...
                    dx2 = (position_cpp[ 4 * i + 0 ] - position_cpp[ 4 * j + 0 ]);
                    dx2 *= dx2;
                    dy2 = (position_cpp[ 4 * i + 1 ] - position_cpp[ 4 * j + 1 ]);
                    dy2 *= dy2;
                    dz2 = (position_cpp[ 4 * i + 2 ] - position_cpp[ 4 * j + 2 ]);
                    dz2 *= dz2;
                    r2_ij = dx2 + dy2 + dz2;
                                ...
                                if( (fabs(position_cpp[4*i+3]-2.2f)<=0.05) && (fabs(position_cpp[4*j+3]-2.2f)<=0.05) )//both points - i and j - are green
                                    if((dz2>4*dx2)&&(dz2>4*dy2)&&(dx2>4*dy2))
                                    {
                                        if(position_cpp[ 4 * i + 0 ]>WXC)
                                        {
                                            ...
                                            {
                                                if(dq==1) q_i_start = 0;
                                                else q_i_start = 24*3; //muscle quadrant starting index
                                        ...
                                        else
                                        {
                                            ...
                                            for(dq=-1; dq<=1; dq+=2) //dorsal quadrant - "-1"=right, "+1"=left
                                            {
                                                if(dq==1) q_i_start = 24;
                                                else q_i_start = 24*2; //muscle quadrant starting index
